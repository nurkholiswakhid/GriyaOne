...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-md p-8 fade-in">
        <form action="{{ route('informasi.update', $informasi->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                @if($informasi->photo)
                    <div id="photoCurrentContainer" class="mb-4">
                        <p class="text-xs text-gray-500 mb-2">Foto Saat Ini:</p>
                        <img id="photoCurrentImage" src="{{ asset('storage/' . $informasi->photo) }}" alt="{{ $informasi->title }}" class="w-full h-48 object-cover rounded-lg border border-gray-300">
                    </div>
                @endif
                <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-red-500 hover:bg-red-50 transition" id="photoDropZone">
                    ...
                    <div id="photoDropContent">
                        ...
                        <p class="text-gray-700 font-medium">Klik atau drag foto baru (opsional)</p>
                ...
                <div class="flex gap-2">
                    <select id="category_id" name="category_id" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition @error('category_id') border-red-500 @enderror">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $informasi->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
...
function previewPhoto(file) {
    ...
    reader.onload = function(e) {
        ...
        const currentContainer = document.getElementById('photoCurrentContainer');
