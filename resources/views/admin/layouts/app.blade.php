...
<body class="bg-gray-50">
    <!-- Navbar -->
    <nav class="sticky top-0 z-50 bg-white shadow-sm border-b border-red-100">
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center gap-3 min-w-0">
                    <div class="bg-red-600 hover:bg-red-700 w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold text-lg flex-shrink-0 transition-colors duration-200">G</div>
                    <div class="hidden sm:block min-w-0">
                        <h1 class="text-lg sm:text-xl font-bold text-gray-900 truncate">GriyaOne</h1>
                        <p class="text-xs text-red-600 font-semibold">@yield('role', 'Admin Dashboard')</p>
                    </div>
                </div>
                <button id="notificationBtn" class="relative p-2 text-gray-600 hover:text-red-600 transition flex-shrink-0">
                ...
                <div id="notificationDropdown" class="hidden absolute top-16 right-4 w-80 bg-white rounded-lg shadow-2xl border border-gray-200 z-40 max-h-96 overflow-y-auto">
                    <div class="bg-gradient-to-r from-red-600 to-red-700 px-4 py-3 sticky top-0 z-10">
    ...
    <div class="flex min-h-screen">
        <!-- Sidebar -->
        <aside class="w-64 bg-gradient-to-b from-white to-red-50 border-r border-gray-200 min-h-screen pt-4 fixed left-0 top-16 bottom-0 overflow-y-auto lg:relative lg:top-0 hidden lg:block scrollbar-hide">
            <!-- User Profile Card -->
            <div class="mx-4 mb-6 bg-white rounded-lg border border-gray-200 p-4 shadow-sm hover:shadow-md transition-shadow duration-200">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @else
                        <div class="w-12 h-12 bg-gradient-to-br from-red-400 to-red-600 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
            ...
            <nav class="px-2 space-y-1">
                <!-- Main Navigation -->
                <a href="{{ route('admin.dashboard') }}" class="bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 flex items-center justify-between gap-3 px-4 py-3 rounded-lg text-white font-medium text-sm shadow-md hover:shadow-lg transition-all duration-200 group">
                    <div class="flex items-center gap-3">
                ...
                <a href="{{ route('users.index') }}" class="group relative flex items-center justify-between px-4 py-3 text-gray-700 hover:text-red-600 hover:bg-red-50 font-medium text-sm transition-colors duration-200 rounded-lg">
                    <div class="flex items-center gap-3">
                    ...
                    <span class="bg-red-100 text-red-600 text-xs font-semibold px-2 py-0.5 rounded-full group-hover:bg-red-200 transition-colors duration-200">{{ \App\Models\User::count() }}</span>
                </a>

                <a href="{{ route('assets.index') }}" class="group relative flex items-center justify-between px-4 py-3 text-gray-700 hover:text-red-600 hover:bg-red-50 font-medium text-sm transition-colors duration-200 rounded-lg">
                    <div class="flex items-center gap-3">
                        <svg class="w-5 h-5 flex-shrink-0 text-gray-400 group-hover:text-red-600 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="bg-red-100 text-red-600 text-xs font-semibold px-2 py-0.5 rounded-full group-hover:bg-red-200 transition-colors duration-200">{{ \App\Models\Asset::count() }}</span>
                </a>

                <a href="{{ route('contents.index') }}" class="group relative flex items-center justify-between px-4 py-3 text-gray-700 hover:text-red-600 hover:bg-red-50 font-medium text-sm transition-colors duration-200 rounded-lg">
                    <div class="flex items-center gap-3">
                        <svg class="w-5 h-5 flex-shrink-0 text-gray-400 group-hover:text-red-600 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('materi.index') }}" class="group relative flex items-center justify-between px-4 py-3 text-gray-700 hover:text-red-600 hover:bg-red-50 font-medium text-sm transition-colors duration-200 rounded-lg">
                    <div class="flex items-center gap-3">
                        <svg class="w-5 h-5 flex-shrink-0 text-gray-400 group-hover:text-red-600 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="bg-red-100 text-red-600 text-xs font-semibold px-2 py-0.5 rounded-full group-hover:bg-red-200 transition-colors duration-200">{{ \App\Models\Content::where('type', 'Materi')->count() }}</span>
                </a>
                ...
                <a href="{{ route('informasi.index') }}" class="group relative flex items-center justify-between px-4 py-3 text-gray-700 hover:text-red-600 hover:bg-red-50 font-medium text-sm transition-colors duration-200 rounded-lg">
                    ...
                    <span class="bg-red-100 text-red-600 text-xs font-semibold px-2 py-0.5 rounded-full group-hover:bg-red-200 transition-colors duration-200">{{ \App\Models\Information::count() }}</span>
                </a>
                ...
                <a href="{{ route('profile.edit') }}" class="group flex items-center gap-3 px-4 py-3 text-gray-700 hover:text-red-600 hover:bg-red-50 font-medium text-sm transition-colors duration-200 rounded-lg">
                    <svg class="w-5 h-5 flex-shrink-0 text-gray-400 group-hover:text-red-600 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
        ...
        function displayNotifications(notifications) {
            ...
            notificationList.innerHTML = notifications.map(notif => `
                <div class="px-4 py-3 hover:bg-red-50 transition-colors cursor-pointer">
