...
@section('content')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="col-span-2">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <input type="url" name="gmap_link" value="{{ old('gmap_link', $asset->gmap_link) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-red-500 transition @error('gmap_link') border-red-500 @enderror" placeholder="https://maps.google.com/...">
                                </div>
                                @error('gmap_link')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                ...
                @if($asset->photos && count($asset->photos) > 0)
                    <div class="bg-white rounded-xl p-6 shadow-md">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-bold text-gray-900">Foto Saat Ini</h3>
                            <span class="text-xs text-gray-500 bg-gray-100 px-3 py-1.5 rounded-full font-semibold">
                                {{ count($asset->photos) }} foto
                            </span>
                        </div>
                        <p class="text-xs text-gray-500 mb-4">Klik tombol <span class="text-red-600 font-semibold">Hapus</span> pada foto yang ingin dihapus. Perubahan akan disimpan saat klik "Perbarui Aset".</p>
                            ...
                            @foreach($asset->photos as $index => $photo)
                                <div class="relative rounded-xl border border-gray-200 bg-white shadow-sm photo-item" data-photo="{{ $photo }}" style="overflow:hidden;">
                                    ...
                                    <div class="flex items-center justify-between px-3 py-2 bg-gray-50 border-t border-gray-200">
                                        <span class="text-xs text-gray-500 font-medium">Foto {{ $index + 1 }}</span>
                                        <button type="button"
                                            onclick="deleteExistingPhoto('{{ $photo }}', this)"
                                            class="inline-flex items-center gap-1 bg-red-600 hover:bg-red-700 text-white text-xs font-bold px-3 py-1 rounded-lg transition active:scale-95">
                                    ...
                                    <div class="deleted-overlay hidden absolute inset-0 bg-red-800/80 flex flex-col items-center justify-center" style="z-index:10;">
                                        <svg class="w-7 h-7 text-white mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                        <p class="text-white text-xs font-bold mb-2">Akan Dihapus</p>
                                        <button type="button" onclick="undoDeletePhoto('{{ $photo }}', this)"
                                            class="bg-white text-red-700 hover:bg-red-50 text-xs font-bold px-3 py-1 rounded-lg transition">
                                            Batalkan
                                        </button>
                                    </div>
                                </div>
                ...
                <div class="flex gap-4">
                    ...
                    <a href="{{ route('assets.show', $asset) }}" id="cancel-btn" class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-900 px-6 py-3 rounded-lg font-semibold transition text-center">
                        Batalkan
                    </a>
                </div>
    ...
    <script>
        ...
        function deleteExistingPhoto(photoPath, button) {
            if (!confirm('Hapus foto ini? Perubahan disimpan saat klik "Perbarui Aset".')) return;
            const photoItem = button.closest('.photo-item');
            const overlay   = photoItem.querySelector('.deleted-overlay');
            const bar       = button.closest('div'); // bar bawah
            pendingDeletes.add(photoPath);
            syncDeletedInputs();
            if (overlay) overlay.classList.remove('hidden');
            if (bar) bar.classList.add('hidden'); // sembunyikan bar bawah
        }

        function undoDeletePhoto(photoPath, undoBtn) {
            pendingDeletes.delete(photoPath);
            syncDeletedInputs();
            const photoItem = undoBtn.closest('.photo-item');
            const overlay   = photoItem.querySelector('.deleted-overlay');
            // Cari bar bawah (div dengan flex items-center justify-between)
            const bar = photoItem.querySelector('[onclick^="deleteExistingPhoto"]')?.closest('div');
            if (overlay) overlay.classList.add('hidden');
            if (bar) bar.classList.remove('hidden');
        }

        function syncDeletedInputs() {
            document.querySelectorAll('input[name="deleted_photos[]"]').forEach(i => i.remove());
            const form = document.querySelector('form');
            pendingDeletes.forEach(path => {
                const inp = document.createElement('input');
                inp.type = 'hidden'; inp.name = 'deleted_photos[]'; inp.value = path;
        ...
        window.handleFormSubmit = function(event) {
            ...
            // 2. Build FormData (includes all hidden inputs for deleted_photos[])
            ...
            function resetSubmitButton() {
                if (submitBtn) {
                    ...
                    submitBtn.innerHTML = `<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"/></svg> Perbarui Aset`;
                }
