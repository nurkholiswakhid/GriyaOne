    ...
    <form id="asset-form" action="{{ route('assets.store') }}" method="POST" enctype="multipart/form-data" class="fade-in" onsubmit="return handleFormSubmit(event)">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl p-6 shadow-md">
                    ...
                    <!-- Upload cards (per file progress) -->
        ...
        (function () {
            ...
            function renderCard(item) {
                ...
                card.innerHTML = `
                    <img src="${item.thumbUrl}" class="w-full h-28 object-cover" alt="" decoding="async">
                    <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent pointer-events-none"></div>
                    <div class="absolute bottom-0 left-0 right-0 p-1.5 pointer-events-none">
                        <p class="text-white text-[9px] truncate">${item.file.name}</p>
                        <p class="text-white/60 text-[8px]">${(item.file.size/1024).toFixed(0)} KB</p>
                    </div>
                    ${statusHtml}`;
            }
