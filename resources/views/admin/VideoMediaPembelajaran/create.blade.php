...
@section('title', 'Tambah Video Pembelajaran - GriyaOne')
@section('role', 'Tambah Video Pembelajaran')

@section('content')
    <!-- Header -->
    <div class="mb-8 flex justify-between items-start fade-in">
        <div>
            <h2 class="text-3xl font-bold text-gray-900 mb-1">Tambah Video Pembelajaran</h2>
            <p class="text-gray-600">Tambahkan video pembelajaran baru dari YouTube</p>
        </div>
        <a href="{{ route('contents.index') }}" class="text-red-600 hover:text-red-700 font-medium text-sm transition">← Kembali ke Daftar</a>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl p-6 shadow-md">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-red-500 transition @error('title') border-red-500 @enderror" placeholder="Contoh: Teknik Closing Penjualan Properti">
                            @error('title')
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-red-500 transition @error('description') border-red-500 @enderror" placeholder="Jelaskan isi video secara singkat...">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="bg-white rounded-xl p-6 shadow-md">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="url" name="file_path" value="{{ old('file_path') }}" id="youtube_url" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-red-500 transition @error('file_path') border-red-500 @enderror" placeholder="https://www.youtube.com/watch?v=dQw4w9WgXcQ">
                            <p class="text-gray-500 text-xs mt-2">Masukkan link YouTube lengkap. Contoh: https://www.youtube.com/watch?v=dQw4w9WgXcQ</p>
                            @error('file_path')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div id="youtube-preview" class="hidden">
                            ...
                            <div class="rounded-xl overflow-hidden border border-gray-200 bg-black aspect-video shadow-sm">
                                <iframe id="youtube-iframe" class="w-full h-full" frameborder="0" allowfullscreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>
                            </div>
                ...
                <div class="bg-white rounded-xl p-6 shadow-md">
                    ...
                    <p class="text-gray-500 text-xs mb-4">Jika tidak diupload, thumbnail otomatis diambil dari YouTube</p>
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-red-500 hover:bg-red-50 transition" id="thumbnail-dropzone">
                        ...
                        <p class="text-gray-400 text-xs">JPG, PNG, GIF (Maks 2MB)</p>
                        <input type="file" name="thumbnail" accept="image/*" class="hidden" id="thumbnail-input">
                        <button type="button" class="mt-3 bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-1.5 rounded-lg font-medium text-xs transition" onclick="document.getElementById('thumbnail-input').click()">Pilih Gambar</button>
                    </div>
                ...
                <div class="bg-white rounded-xl p-6 shadow-md">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_published" value="1" @checked(old('is_published') !== false) class="w-4 h-4 text-red-600 rounded focus:ring-2 focus:ring-red-500">
                        <div>
            ...
            <div class="lg:col-span-1">
                <div class="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-6 border border-red-200 sticky top-24">
                    ...
                    <div class="space-y-3 text-sm text-gray-700">
                        <div>
                            <p class="font-semibold text-gray-900">1. Link YouTube</p>
                            <p class="text-xs text-gray-600 mt-1">Pastikan video sudah diupload di YouTube dan bersifat Public atau Unlisted.</p>
                        </div>
                        <div class="border-t border-red-200 pt-3">
                            <p class="font-semibold text-gray-900">2. Judul yang Jelas</p>
                            <p class="text-xs text-gray-600 mt-1">Gunakan judul deskriptif agar mudah ditemukan oleh tim.</p>
                        </div>
                        <div class="border-t border-red-200 pt-3">
                            <p class="font-semibold text-gray-900">3. Thumbnail</p>
                            <p class="text-xs text-gray-600 mt-1">Opsional — jika tidak diupload, thumbnail YouTube otomatis digunakan.</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        if (youtubeUrl.value) {
            youtubeUrl.dispatchEvent(new Event('input'));
        }
