    ...
    <nav role="navigation" aria-label="Pagination Navigation">
        ...
        <div class="flex items-center justify-between sm:hidden">
            <div class="flex items-center gap-2">
                ...
                @else
                    <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="inline-flex items-center gap-1.5 px-3.5 py-2 text-sm font-medium text-orange-600 bg-white border border-gray-200 rounded-lg hover:bg-orange-50 hover:border-orange-200 active:bg-orange-100 transition duration-200 shadow-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
                        Sebelumnya
                    </a>
                @endif
            ...
            <div class="flex items-center gap-2">
                @if ($paginator->hasMorePages())
                    <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="inline-flex items-center gap-1.5 px-3.5 py-2 text-sm font-medium text-orange-600 bg-white border border-gray-200 rounded-lg hover:bg-orange-50 hover:border-orange-200 active:bg-orange-100 transition duration-200 shadow-sm">
                        Berikutnya
                        ...
                        @foreach ($element as $page => $url)
                            @if ($page == $paginator->currentPage())
                                <span aria-current="page">
                                    <span class="inline-flex items-center justify-center w-10 h-10 text-sm font-bold text-white bg-gradient-to-br from-orange-500 to-orange-600 rounded-lg shadow-md shadow-orange-200 cursor-default select-none">{{ $page }}</span>
                                </span>
                            @else
